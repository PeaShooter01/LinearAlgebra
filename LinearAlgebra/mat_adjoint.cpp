...
Mat_Adjoint::Mat_Adjoint(QWidget *parent) : QWidget(parent)
{
    setWindowTitle("求伴随矩阵");
        ...
        else
        {
            try
            {
                ...
                for(int j=0;j<a.get_row_num()-1;++j)
                    n+=a.get_mat_adjoint()[0][j].to_QString()+" ";
                n+=a.get_mat_adjoint()[0][a.get_row_num()-1].to_QString();
                for(int i=1;i<a.get_colomn_num();++i)
                {
                    n+='\n';
                    for(int j=0;j<a.get_row_num()-1;++j)
                        n+=a.get_mat_adjoint()[i][j].to_QString()+" ";
                    n+=a.get_mat_adjoint()[i][a.get_row_num()-1].to_QString();
                }
                lab->setText(n);
            }
