...
Mat_Simp::Mat_Simp(QWidget *parent) : QWidget(parent)
{
    setWindowTitle("化阶梯形矩阵");
        ...
        else
        {
            try
            {
                ...
                for(int j=0;j<a.get_colomn_num()-1;++j)
                    n+=a.get_mat_echelon()[0][j].to_QString()+" ";
                n+=a.get_mat_echelon()[0][a.get_colomn_num()-1].to_QString();
                for(int i=1;i<a.get_row_num();++i)
                {
                    n+='\n';
                    for(int j=0;j<a.get_colomn_num()-1;++j)
                        n+=a.get_mat_echelon()[i][j].to_QString()+" ";
                    n+=a.get_mat_echelon()[i][a.get_colomn_num()-1].to_QString();
                }
                lab->setText(n);
            }
